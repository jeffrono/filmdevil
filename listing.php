...
?>
    <tr id="tableRow" onClick="selectRow(this.rowIndex, '<?= $selectedClass ?>', '<?= $normalClass ?>');"
			...
			class="<?= $normalClass ?>">
      <td width="21" bgcolor="black" align="center" vAlign="center">
				<img src="images/icon<? print round($row["rating"]) ?>.gif"
	  			alt="<? if (round($row["rating"]) == 0) { ?>Not Yet Rated<? } else { print round($row["rating"]); ?> out of 5<? } ?>">
			</td>
      <td>
        <a id="titleLink" href="action.php?type=2&id=<? print $row["ID"] ?>" target="infoFrame">
<?if($showLogo) {
		$logoSize = getScaledImageSize($row["logoURL"], 80, 80); ?>
					<img align="middle" border="0" scr="<?= $row["logoURL"] ?>" height="<?= $logoSize["height"] ?>"
						width="<?= $logoSize["width"] ?>">
<?} ?>
				<?= limitWordLength($row["title"], 16) ?></a>
      </td>
...
<? if(empty($_GET) && empty($_POST)) { ?>
	<body background="images/bg.gif">
		<p style="font-family:trebuchet ms, arial; color:#FFFFFF; font-size: 18px; padding: 0px; margin: 10px;">
			Once you make your search on the left,
			the results will be shown here</p>
	</body></html>
<?		die;
	}
?>
